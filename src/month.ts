...
import { RenderInfo, MonthInfo, ChartElements, ValueType } from './data';
...
function renderMonthHeader(
  ...
  const weekDays = chartElements.graphArea
    .selectAll('weekDays')
    .data(weekdayNames)
    .enter()
    .append('text')
...
function renderMonthDays(
  ...
  if (mode === 'circle' && monthInfo.showSelectedRing) {
    const selectedRings = chartElements.dataArea
  ...
  // labels
  const dayLabals = chartElements.dataArea
    .selectAll('dayLabel')
    .data(daysInMonthView)
    .enter()
    .append('text')
  ...
  if (mode === 'annotation' && showAnnotation) {
    const dayAnnotation = chartElements.dataArea
