...
function renderLegend(
  ...
  const nameSizes = names.map(function (n) {
  ...
  const firstLabelX = firstMarkerX + xSpacing + markerWidth; // xSpacing + 2 * xSpaing
...
function renderPie(
  ...
  const outterRadius = radius * 0.7;
  const innerRadius = outterRadius * pieInfo.ratioInnerRadius;
  ...
  // label sizes
  const labelSizes = labels.map(function (n) {
  ...
  // extLabel sizes
  const extLabelSizes = extLabels.map(function (n) {
  ...
  const arc = d3.arc().innerRadius(innerRadius).outerRadius(outterRadius);
